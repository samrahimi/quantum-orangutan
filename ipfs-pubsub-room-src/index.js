...
class PubSubRoom extends EventEmitter {
  ...
  _emitChanges (newPeers) {
    ...
    //when we poll in future, any changes will be legitimate
  ...
  _handleDirectMessage (message) {
    console.log(`ipfs-pub-sub: direct message on ${_this.topic} with contents ${JSON.stringify(message, null, 2)}`)
    if (message.to === this._ipfs._peerInfo.id.toB58String()) {
      console.log("message is for me, emitting direct-message event")
      const m = Object.assign({}, message)
      delete m.to
      this.emit('direct-message', m)
    } else {
      console.log("not the recipient, will not handle")
    }
  }
}
